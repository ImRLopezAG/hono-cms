...
/**
 * Build a drizzle-backed `AuditStore` for either the `sqlite` or `postgres`
 * dialect. The table schema is declared inline so callers don't need to wire
 * a separate migration — just pass a drizzle `db` whose pragma includes a
 * table named {@link CreateDrizzleAuditStoreOptions.tableName} (default
 * `audit_log`).
 */
export function createDrizzleAuditStore(
  db: any,
  options: CreateDrizzleAuditStoreOptions
): AuditStore {
  ...
  return {
    ...
    async health(): Promise<HealthStatus> {
      try {
        if (typeof db.execute === "function") await db.execute(sql`SELECT 1`);
        else if (typeof db.run === "function") await db.run(sql`SELECT 1`);
        return { ok: true, details: { table: tableName } };
      } catch (error) {
        return {
          ok: false,
          details: { table: tableName, error: error instanceof Error ? error.message : String(error) }
        };
      }
    }
  };
