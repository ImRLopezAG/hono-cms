/**
 * `createPluginCMS` — the plugin-manifest kernel.
 *
 * Replacement for the legacy `createCMS` monolith (still present in
 * `create-cms.ts` pending U23 deletion). Takes a `plugins: Plugin[]`
 * array, mounts the minimum content REST surface, and wires the
 * AuthPlugin's `protected` middleware + `authorize` policy around it.
 *
 * The kernel itself stays under ~300 lines — everything cross-cutting
 * lives in plugins.
 */
...
export type PluginCMSConfig<Collections extends CMSCollections = CMSCollections> = {
...
export type PluginCMSInstance<Collections extends CMSCollections = CMSCollections> =
  Hono<HonoCMSEnv> & {
...
export async function createPluginCMS<Collections extends CMSCollections>(
  config: PluginCMSConfig<Collections>
): Promise<PluginCMSInstance<Collections>> {
  ...
  // Health endpoints — always mounted by the kernel.
  ...
  // Content REST surface — protected by the AuthPlugin + authorized via
  // `installed.authorize`. Mounted only for collections the user declared.
  ...
  // Assemble the public instance — Hono app plus introspection handles.
  const instance = app as PluginCMSInstance<Collections>;
