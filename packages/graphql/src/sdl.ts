 ...
 *
 * Ported verbatim from the legacy `packages/core/src/graphql.ts`. Builds the
 * textual SDL served at `GET /graphql/schema` (and the legacy
 * `/cms/graphql/schema` alias). The same string is also fed into
 * `makeExecutableSchema` by {@link buildGraphQLSchema} so the served SDL and
 * the executed schema stay in lockstep.
 */
